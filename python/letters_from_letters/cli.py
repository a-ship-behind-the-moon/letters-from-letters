...
def main(argv=None):
    ...
    if len(argv) > 3 and argv[1].lower() in ("random", "rand", "rng"):
        if all(n in string.digits for n in argv[2]):
            return tuple(parser.random_n(char, int(argv[2])) for char in argv[3:])
        return tuple(parser.random_n(char) for char in argv[2:])
